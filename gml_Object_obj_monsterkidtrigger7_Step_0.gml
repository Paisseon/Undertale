if (con == 1)
{
    mkid = instance_create((__view_get(0, 0) - 40), obj_mainchara.y, obj_mkid_actor)
    mkid.image_speed = 0
...
{
    undyne = instance_create((__view_get(0, 0) - 40), 82, obj_undynea_actor)
...
if (con == 34)
{
    ...
    con = 35
    global.flag[17] = 1
    doorb = instance_create((__view_get(0, 0) - 20), (obj_mainchara.y + 10), obj_doorB)
    doora = instance_create(((__view_get(0, 0) + __view_get(2, 0)) + 20), (obj_mainchara.y + 10), obj_doorA)
    undynetimer = 0
    finaltimer = 0
    mkidtalk = 0
    charge = 0
...
{
    con = 73
    idealxview = round(((obj_mainchara.x - (__view_get(2, 0) / 2)) + (obj_mainchara.sprite_width / 2)))
    if (idealxview >= (room_width - __view_get(2, 0)))
        idealxview = ((room_width - __view_get(2, 0)) - 2)
    if (__view_get(0, 0) > idealxview)
        xdir = 0
    else
        xdir = 1
    alarm[4] = 40
...
if (con == 74)
{
    if (xdir == 1)
        ...
        __view_set(0, 0, (__view_get(0, 0) - 4))
    if (abs((__view_get(0, 0) - idealxview)) <= 5)
    {
        with (mkid)
            instance_destroy()
        __view_set(9, 0, 1584)
...
if (con == 117)
{
    if (xdir == 1)
        ...
        __view_set(0, 0, (__view_get(0, 0) - 2))
    if (abs((__view_get(0, 0) - idealxview)) <= 2)
    {
        with (mkid)
            instance_destroy()
        __view_set(9, 0, 1584)
