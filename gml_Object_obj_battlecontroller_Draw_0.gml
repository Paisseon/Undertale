...
if ((global.bmenuno == 1) && ((global.myfight == 0) && (global.mnfight == 0)))
{
    ...
    for (i = 0; i < 3; i++)
    {
        if ((global.monster[i] == true) && (!instance_exists(obj_sansb)))
        {
            ...
            if (global.language == "ja")
                lineheight = 36
            draw_rectangle(xwrite, (y_start + (i * lineheight)), (xwrite + 100), ((y_start + (i * lineheight)) + 16), 0)
            draw_set_color(c_lime)
            draw_rectangle(xwrite, (y_start + (i * lineheight)), (xwrite + ((global.monsterhp[i] / global.monstermaxhp[i]) * 100)), ((y_start + (i * lineheight)) + 16), 0)
        }
    }
}
