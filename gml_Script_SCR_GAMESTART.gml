...
if global.debug
{
    instance_create(0, 0, obj_debugcontroller)
    if file_exists("debug.txt")
        file_delete("debug.txt")
    date_set_timezone(1)
    var date = date_current_datetime()
    debug_message(((((((((((("date: " + string(date_get_day(date))) + "/") + string(date_get_month(date))) + "/") + string(date_get_year(date))) + " ") + string(date_get_hour(date))) + ":") + string(date_get_minute(date))) + ":") + string(date_get_second(date))))
    debug_message("build: v2.0.0-noswitch Nov 25 2019 21:43:36")
}
