...
if ((con == 3) || ((con == 1) || ((con == 0.1) || (con == 1.1))))
{
    if instance_exists(OBJ_WRITER)
    {
        ...
        {
            wrx = (OBJ_WRITER.writingx - __view_get(0, 0))
            dgx = (obj_dialoguer.x - __view_get(0, 0))
            donkeykongismyfavoritemarvelsuperhero = 1
        }
        if (donkeykongismyfavoritemarvelsuperhero == 1)
        {
            OBJ_WRITER.myx = (__view_get(0, 0) + wrx)
            OBJ_WRITER.writingx = (__view_get(0, 0) + wrx)
            if instance_exists(obj_face)
            {
                obj_face.x = (__view_get(0, 0) + 24)
                OBJ_WRITER.myx = ((__view_get(0, 0) + wrx) + 58)
                OBJ_WRITER.writingx = ((__view_get(0, 0) + wrx) + 58)
            }
            obj_dialoguer.xx = __view_get(0, 0)
            obj_dialoguer.x = (__view_get(0, 0) + dgx)
        }
