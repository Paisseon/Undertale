...
while ((global.idealborder[0] + (i * 5)) < global.idealborder[1])
{
    ...
    if collision_line(toothxx[i], global.idealborder[2], toothxx[i], (toothyy[i] - 3), obj_heart, 0, 1)
        event_user(1)
    if collision_line(toothxx[i], global.idealborder[3], toothxx[i], ((toothyy[i] + toothdist) + 3), obj_heart, 0, 1)
        event_user(1)
    i += 1
}
