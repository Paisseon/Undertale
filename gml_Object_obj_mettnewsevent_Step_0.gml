...
{
    wordfall = instance_create((__view_get(0, 0) + 20), (__view_get(1, 0) + 20), obj_wordfall)
    global.typer = 5
...
if ((con == 131) && (instance_exists(OBJ_WRITER) == 0))
{
    ...
    newgame.bombtype = 6
    newticker = instance_create(__view_get(0, 0), (__view_get(1, 0) + 200), obj_mettnews_ticker)
    newticker.on = 1
    newticker.doom = 1
    newticker.voff = 40
...
if (con == 162)
{
    ...
    with (newticker)
        instance_destroy()
    mett = instance_create((__view_get(0, 0) + 340), (__view_get(1, 0) + 70), obj_mettanchor)
    global.flag[20] = 8
    mett.on = 2
