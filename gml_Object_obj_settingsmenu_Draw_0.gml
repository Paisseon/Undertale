...
if (global.osflavor >= 3)
{
    ...
    if (global.osflavor >= 4)
    {
        ...
        for (i = 0; i < 3; i++)
        {
            ...
            draw_set_color(c_white)
            xx = (40 + string_width(string_hash_to_newline(itext)))
            if (xx > button_x)
                button_x = xx
        }
        button_x += 15
    ...
    else
    {
        if (menu == 2)
            ...
            draw_set_color(c_white)
        draw_text(20, 100, string_hash_to_newline(scr_gettext("settings_button_opacity")))
        var analog_text_width = string_width(string_hash_to_newline(scr_gettext("settings_button_opacity")))
        var sensebar_x = ((20 + analog_text_width) + 5)
        if ((menu == 2) && (menu_engage == 1))
        {
            if (global.debug == 1)
            {
                draw_set_font(fnt_maintext)
                draw_text(10, 50, string(global.controller_opacity))
            }
            draw_sprite_ext(spr_sensing_slider, 0, sensebar_x, 100, 1, 1, 0, c_red, 1)
            draw_sprite_ext(spr_sensebar, 0, (sensebar_x + (global.controller_opacity * 38)), 103, 1, 1, 0, c_yellow, 1)
            if (buffer < 0)
            {
                if obj_time.right
                    global.controller_opacity += 0.04
                if obj_time.left
                    global.controller_opacity -= 0.04
                if (global.controller_opacity >= 1)
                    global.controller_opacity = 1
                if (global.controller_opacity <= 0)
                    global.controller_opacity = 0
        ...
        {
            draw_sprite_ext(spr_sensing_slider, 0, sensebar_x, 100, 1, 1, 0, c_red, 0.6)
            draw_sprite_ext(spr_sensebar, 0, (sensebar_x + (global.controller_opacity * 38)), 103, 1, 1, 0, c_yellow, 0.6)
        }
        if (menu == 3)
            draw_set_color(c_yellow)
        else
            draw_set_color(c_white)
        draw_text(20, 124, string_hash_to_newline(scr_gettext("settings_mcontroller_deadzone")))
        analog_text_width = string_width(string_hash_to_newline(scr_gettext("settings_mcontroller_deadzone")))
        sensebar_x = ((20 + analog_text_width) + 5)
        if ((menu == 3) && (menu_engage == 1))
        {
            if (global.debug == 1)
            {
                draw_set_font(fnt_maintext)
                draw_text(10, 70, string(global.controller_deadzoner))
            }
            obj_mobilecontroller.dzdraw = 1
            draw_sprite_ext(spr_sensing_slider, 0, sensebar_x, 124, 1, 1, 0, c_red, 1)
            draw_sprite_ext(spr_sensebar, 0, (sensebar_x + (global.controller_deadzoner * 38)), 127, 1, 1, 0, c_yellow, 1)
            if (buffer < 0)
            {
                if obj_time.right
                    global.controller_deadzoner += 0.04
                if obj_time.left
                    global.controller_deadzoner -= 0.04
                if (global.controller_deadzoner >= 1)
                    global.controller_deadzoner = 1
                if (global.controller_deadzoner <= 0)
                    global.controller_deadzoner = 0
                if (control_check_pressed(0) || control_check_pressed(1))
                {
                    menu_engage = 0
                    obj_mobilecontroller.dzdraw = 0
                }
            }
        }
        else
        {
            draw_sprite_ext(spr_sensing_slider, 0, sensebar_x, 124, 1, 1, 0, c_red, 0.6)
            draw_sprite_ext(spr_sensebar, 0, (sensebar_x + (global.controller_deadzoner * 38)), 127, 1, 1, 0, c_yellow, 0.6)
        }
        if (menu == 4)
            draw_set_color(c_yellow)
        else
            draw_set_color(c_white)
        draw_text(20, (100 + (3 * vspacing)), string_hash_to_newline(scr_gettext("joyconfig_test")))
        if ((menu == 4) && (menu_engage == 1))
        {
            caster_free(-3)
